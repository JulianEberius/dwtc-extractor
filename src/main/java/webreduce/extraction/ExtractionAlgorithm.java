...
public class ExtractionAlgorithm {
	...
	protected static final int MAX_NEXT_AND_PREVIOUS_CONTENT = 1000;
	...
	public static enum TABLE_COUNTERS {
		TABLES_FOUND, TABLES_INSIDE_FORMS, NON_LEAF_TABLES, SMALL_TABLES, RELATIONS_FOUND, SPARSE_TABLE, LINK_TABLE, CALENDAR_FOUND, NON_REGULAR_TABLES, LANGDETECT_EXCEPTION, ENGLISH, NON_ENGLISH, TO_MANY_BADWORDS, SPANNING_TD, NO_HEADERS, MORE_THAN_ONE_HEADER, SHORT_ATTRIBUTE_NAMES, LONG_ATTRIBUTE_NAMES, CW_CRASHED, CW_NOT_EXACTLY_ONE_TABLE, MAUI_EXCEPTION
	}

	public ExtractionAlgorithm(StatsKeeper stats, boolean th_extract_terms) {
		super();
		this.stats = stats;
	...
	public List<Dataset> extract(Document doc, DocumentMetadata metadata) throws IOException,
		...
		main_loop: for (Element table : doc.getElementsByTag("table")) {
			...
			// stats.reportProgress();
			Optional<Dataset> r = doExtract(table, trs,
	...
	protected Optional<Dataset> asRelation(Elements input,
			int numCols, int nullLimit, int linkLimit) {
		int nullCounter = 0;
		int linkCounter = 0;
